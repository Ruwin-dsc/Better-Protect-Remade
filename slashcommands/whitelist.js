...
module.exports = {
  ...
  async run(bot, message, args) {
    ...
    bot.db.query(`SELECT * FROM whitelist WHERE guildId = "${message.guild.id}"`, async (err, req) => {
    ...
    collector.on('collect', async (interaction) => {
      ...
      msg.edit({ components: [rows, rows2] })        
        ...
        interaction.values.forEach(id => {
          bot.db.query(`SELECT * FROM whitelist WHERE guildId = "${message.guild.id}" AND userId = "${id}"`, async (err, req) => {
            if(req.length < 1) bot.db.query(`INSERT INTO whitelist (guildId, userId) VALUES (${message.guild.id}, ${id})`)
          })
        })

        await wait(500)
        await bot.db.query(`SELECT * FROM whitelist WHERE guildId = "${message.guild.id}"`, async (err, req) => {
        embeds = []
            for (let i = 0; i < req.length; i += itemsPerPage) {
              const embed = new Discord.EmbedBuilder()
                .setAuthor({ name: `・Voici la liste blanche du serveur (${req.length})`})
        ...
        interaction.deferUpdate()
          interaction.values.forEach(id => {
            bot.db.query(`DELETE FROM whitelist WHERE userId = ${id}`);
          })
        ...
        if(req.length < 1) {
          ...
          if(req.length < 1) button3.setDisabled(true)
            ...
            rows2 = new Discord.ActionRowBuilder().addComponents(button5)
            await wait(500)
            msg.edit({ embeds: [embeds[0]], components: [rows, rows2] })
          })
      } else if(interaction.customId == "deleteall") {
        interaction.deferUpdate()
        bot.db.query(`DELETE FROM whitelist WHERE guildId = ${message.guild.id}`);
        await wait(500)
        await bot.db.query(`SELECT * FROM whitelist WHERE guildId = "${message.guild.id}"`, async (err, req) => {
        embeds = []
        const embed = new Discord.EmbedBuilder()
