...
module.exports = {
  ...
  async execute(oldMember, newMember, bot) {
    ...
    bot.db.query(`SELECT * FROM antiroleadd WHERE guildId = "${oldMember.guild.id}"`, async (err, req) => {
        ...
        const checkAction = async () => {
          ...
          bot.db.query(`SELECT * FROM logstxt WHERE guildId = "${oldMember.guild.id}"`, async (err, req) => {
              if(req.length ? req[0]?.addrole : "a" == "msg") msg = req[0].text
              bot.db.query(`SELECT * FROM antiroleaddrole WHERE guildId = "${oldMember.guild.id}"`, async (err, req) => {
                ...
                bot.db.query(`SELECT * FROM antiroleaddperms WHERE guildId = "${oldMember.guild.id}"`, async (err, req) => {
        ...
        switch (reste) {
          ...
          case "blanche":
            ...
            break;
          case "independant":
            if (await checkuserr("antiroleaddauser")) return;
            await checkAction();
            break;
          case "proprioindependant":
            if (await checkuserr("ownerlist") || await checkuserr("antiroleaddauser")) return;
            await checkAction();
            break;
          case "blancheindependant":
            if (await checkuserr("whitelist") || await checkuserr("antiroleaddauser")) return;
            await checkAction();
            break;
          case "proprioblancheindependant":
            if (await checkuserr("ownerlist") || await checkuserr("antiroleaddauser") || await checkuserr("whitelist")) return;
            await checkAction();
            break;
          default:
            break;
