...
module.exports = {
  ...
  async execute(oldState, newState, bot) {
    ...
    bot.db.query(`SELECT * FROM depluser WHERE guildId = "${newState.guild.id}"`, async (err, req) => {
      if (err || req.length < 1) return;

      const { state, logs, punition, permission, reste, intervalle, maximum } = req[0];

      const action = await newState.guild.fetchAuditLogs({ limit: 1, type: Discord.AuditLogEvent.MemberMove }).then(audit => audit.entries.first());
      ...
      const checkAction = async () => {
        ...
        bot.db.query(`SELECT * FROM logstxt WHERE guildId = "${newState.guild.id}"`, async (err, req) => {
            if(req.length ? req[0]?.moovuser : "a" == "msg") msg = req[0].text
            if(state == "on") {
            ...
            if(punition == "expul") newState.guild.members.cache.get(action.executor.id).kick({ reason: 'Antidepluser by beter /uhq'})
            else if(punition == "banni") newState.guild.members.cache.get(action.executor.id).ban({ reason: 'Antidepluser by beter /uhq' })
            else if(punition == "supprrole") newState.guild.members.cache.get(action.executor.id).roles.set([], `Antidepluser by beter /uhq}`);
      ...
      switch (reste) {
        ...
        case "blanche":
          ...
          break;
        case "independant":
          if (await checkuserr("depluserlistuser")) return;
          await checkAction();
          break;
        case "proprioindependant":
          if (await checkuserr("ownerlist") || await checkuserr("depluserlistuser")) return;
          await checkAction();
          break;
        case "blancheindependant":
          if (await checkuserr("whitelist") || await checkuserr("depluserlistuser")) return;
          await checkAction();
          break;
        case "proprioblancheindependant":
          if (await checkuserr("ownerlist") || await checkuserr("depluserlistuser") || await checkuserr("whitelist")) return;
          await checkAction();
          break;
        default:
          break;
