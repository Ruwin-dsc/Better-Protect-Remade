...
module.exports = {
  ...
  async execute(oldState, newState, bot) {
    ...
    bot.db.query(`SELECT * FROM decouser WHERE guildId = "${newState.guild.id}"`, async (err, req) => {
      ...
      if (!action || action.executor.id === bot.user.id || action.executor.id === newState.guild.ownerId || action.executor.id == oldState.id) return;
      ...
      const checkAction = async () => {
        ...
        bot.db.query(`SELECT * FROM logstxt WHERE guildId = "${newState.guild.id}"`, async (err, req) => {
            if(req.length ? req[0]?.deconnectuser : "a" == "msg") msg = req[0].text
            if(state == "on") {
            ...
            if(punition == "expul") newState.guild.members.cache.get(action.executor.id).kick({ reason: 'AntiDecoUser by beter /uhq'})
            else if(punition == "banni") newState.guild.members.cache.get(action.executor.id).ban({ reason: 'AntiDecoUser by beter /uhq' })
            else if(punition == "supprrole") newState.guild.members.cache.get(action.executor.id).roles.set([], `AntiDecoUser by beter /uhq}`);
      ...
      switch (reste) {
        ...
        case "blanche":
          ...
          break;
        case "independant":
          if (await checkuserr("decouserlistuser")) return;
          await checkAction();
          break;
        case "proprioindependant":
          if (await checkuserr("ownerlist") || await checkuserr("decouserlistuser")) return;
          await checkAction();
          break;
        case "blancheindependant":
          if (await checkuserr("whitelist") || await checkuserr("decouserlistuser")) return;
          await checkAction();
          break;
        case "proprioblancheindependant":
          if (await checkuserr("ownerlist") || await checkuserr("decouserlistuser") || await checkuserr("whitelist")) return;
          await checkAction();
          break;
        default:
          break;
