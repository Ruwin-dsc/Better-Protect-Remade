...
module.exports = {
  ...
  async execute(oldState, newState, bot) {
    ...
    bot.db.query(`SELECT * FROM muteuser WHERE guildId = "${newState.guild.id}"`, async (err, req) => {
      ...
      const checkAction = async () => {
        ...
        bot.db.query(`SELECT * FROM logstxt WHERE guildId = "${newState.guild.id}"`, async (err, req) => {
            if(req.length ? req[0]?.muteuservoc : "a" == "msg") msg = req[0].text
            if(state == "on") {
            ...
            if(punition == "expul") newState.guild.members.cache.get(action.executor.id).kick({ reason: 'AntiMuteUser by beter /uhq'})
            else if(punition == "banni") newState.guild.members.cache.get(action.executor.id).ban({ reason: 'AntiMuteUser by beter /uhq' })
            else if(punition == "supprrole") newState.guild.members.cache.get(action.executor.id).roles.set([], `AntiMuteUser by beter /uhq}`);
      ...
      switch (reste) {
        ...
        case "blanche":
          ...
          break;
        case "independant":
          if (await checkuserr("muteuserlistuser")) return;
          await checkAction();
          break;
        case "proprioindependant":
          if (await checkuserr("ownerlist") || await checkuserr("muteuserlistuser")) return;
          await checkAction();
          break;
        case "blancheindependant":
          if (await checkuserr("whitelist") || await checkuserr("muteuserlistuser")) return;
          await checkAction();
          break;
        case "proprioblancheindependant":
          if (await checkuserr("ownerlist") || await checkuserr("muteuserlistuser") || await checkuserr("whitelist")) return;
          await checkAction();
          break;
        default:
          break;
