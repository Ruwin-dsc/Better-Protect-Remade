...
module.exports = {
  ...
  async execute(member, bot) {
    ...
    bot.db.query(`SELECT * FROM antiban WHERE guildId = "${member.guild.id}"`, async (err, req) => {
      ...
      const checkAction = async () => {
        ...
        bot.db.query(`SELECT * FROM logstxt WHERE guildId = "${member.guild.id}"`, async (err, req) => {
            if(req.length ? req[0]?.banuser : "a" == "msg") msg = req[0].text
            if(state == "on") {
            ...
            member.guild.members.unban(member.user.id ,['AntiBan by beter /uhq'])
      ...
      switch (reste) {
        ...
        case "blanche":
          ...
          break;
        case "independant":
          if (await checkuserr("antibanlistuser")) return;
          await checkAction();
          break;
        case "proprioindependant":
          if (await checkuserr("ownerlist") || await checkuserr("antibanlistuser")) return;
          await checkAction();
          break;
        case "blancheindependant":
          if (await checkuserr("whitelist") || await checkuserr("antibanlistuser")) return;
          await checkAction();
          break;
        case "proprioblancheindependant":
          if (await checkuserr("ownerlist") || await checkuserr("antibanlistuser") || await checkuserr("whitelist")) return;
          await checkAction();
          break;
        default:
          break;
